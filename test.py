...
def splitDataSetForSeries2(dataSet, axis, value):
...
def createDataSet(filename):
    ...
    dataSet, testdata = train_test_split(dataSet, train_size=0.8, random_state=0)
    ...
    return dataSet, labels, labels2, labels_full, testdata
...
def createTree(dataSet, labels, i):
    ...
    if flagSeries:
        ...
        eltDataSet, gtDataSet = splitDataSetForSeries2(dataSet, bestFeat, midSeries)
...
def ceyan(inputtree,tezhen,shuju):

    root = list(inputtree)[0]

    firstdict = inputtree[str(root)]

    data = dict(zip(tezhen,shuju))

    fen = re.split("小于", root)

    if len(fen) == 2:

        if data[fen[0]] < float(fen[1][:-1]):

            if isinstance(firstdict["小于"],dict):

                label = ceyan(firstdict["小于"],tezhen,shuju)
            ...
            if isinstance(firstdict["大于"],dict):
                ...
                label = ceyan(firstdict["大于"],tezhen,shuju)
        ...
        if data[fen[0]] in firstdict:

            if isinstance(firstdict[data[fen[0]]],dict):

                label = ceyan(firstdict[data[fen[0]]],tezhen,shuju)

            else:

                label = firstdict[data[fen[0]]]
...
filename = r"D:\study\综合课程设计\data\iris.csv"

dataSet, labels, tezhen, labels_full, testdata = createDataSet(filename)
...
for i in range(5):
    ...
    for j in range(int(2/3*len(dataSet))):
...
for i in testdata:
    result = []
    for j in trees:
        result.append(ceyan(j, tezhen, i[:-1]))
